"""Utilities for inspecting BPE vocabulary and merge results."""
...
def print_bpe_result(
    ...
    """Pretty-print a summary of the produced BPE vocabulary and merges."""
...
def _print_vocab_sample(vocab: dict[int, bytes], limit: int = 20) -> None:
    ...
    if vocab:
        ...
        longest_items = sorted(vocab.items(), key=lambda kv: (-len(kv[1]), kv[0]))[
            :top_k
        ]
        print(f"Top {top_k} longest tokens (by bytes):")
        for rank, (tok_id, tok_bytes) in enumerate(longest_items, start=1):
            message = (
                f"  {rank}) id={tok_id}, len={len(tok_bytes)} bytes, "
                f"value={_format_bytes(tok_bytes)}"
            )
            print(message)
